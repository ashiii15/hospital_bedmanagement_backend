...
export const getBeds = async (req:Request,res:Response)=>{
    try{
        ...
        const skip = (page - 1) * limit;
          ...
          const bedList = await Bed.find().skip(skip).limit(limit);
...
export const editBed = async(req:Request,res:Response)=>{
    const {id} = req.params
    const objectId = new mongoose.Types.ObjectId(id);
    const{ward,bedType,isOccupied} = req.body
    
    try{
        const bedDetails = await Bed.findOneAndUpdate({_id:objectId},{ward,bedType,isOccupied},{new:true,runValidators:true})
        if(bedDetails){
            res.status(200).json({
                success:true,
